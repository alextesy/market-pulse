...
# Prometheus metrics
ingest_items_total = Counter("ingest_items_total", "Total ingested items", ["source"])
...
ingest_shard_bytes = Histogram("ingest_shard_bytes", "Shard size in bytes", ["source"])
...
class LakeWriter:
    ...
    def _flush_current_shard(self) -> None:
        ...
        try:
            ...
            # Record metrics
            ingest_shard_bytes.labels(source=self.source_name).observe(len(compressed))
