...
pipeline_items_processed_total = Counter(
    "pipeline_items_processed_total", "Total items processed by pipeline", ["stage", "source"]
)

pipeline_items_failed_total = Counter(
    "pipeline_items_failed_total", "Total items failed in pipeline", ["stage", "source", "reason"]
)

pipeline_processing_latency_seconds = Histogram(
    "pipeline_processing_latency_seconds", "Pipeline processing latency", ["stage", "source"]
)

# Data quality metrics
dq_validation_failures_total = Counter(
    "dq_validation_failures_total", "Total data quality validation failures", ["source", "check"]
)

dq_items_quarantined_total = Counter(
    "dq_items_quarantined_total", "Total items quarantined due to quality issues", ["source", "reason"]
)

# Storage metrics
storage_objects_created_total = Counter(
    "storage_objects_created_total", "Total storage objects created", ["bucket", "prefix"]
)

storage_operations_latency_seconds = Histogram(
    "storage_operations_latency_seconds", "Storage operation latency", ["operation", "bucket"]
)
