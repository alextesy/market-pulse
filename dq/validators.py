...
def validate_text_content(text: str, field_name: str = "text") -> ValidationResult:
    ...
    if text_len < MIN_TEXT_LENGTH:
        result.add_error(
            f"{field_name} too short: {text_len} chars (min {MIN_TEXT_LENGTH})"
        )

    if text_len > MAX_TEXT_LENGTH:
        result.add_error(
            f"{field_name} too long: {text_len} chars (max {MAX_TEXT_LENGTH})"
        )
    ...
    if len(words) > 10:
        ...
        if repetition_ratio > 5.0:
            result.add_error(
                f"{field_name} has excessive word repetition (ratio: {repetition_ratio:.2f})"
            )
...
def compute_title_simhash64(title: str) -> str:
    ...
    word_hashes = [
        hashlib.md5(word.encode()).hexdigest() for word in words[:10]
