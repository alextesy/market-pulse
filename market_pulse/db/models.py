...
from sqlalchemy import (
    BigInteger, String, Text, DateTime, Float, SmallInteger, 
    ForeignKey, UniqueConstraint, Index, CheckConstraint
)
from sqlalchemy.orm import DeclarativeBase, Mapped, mapped_column, relationship
from sqlalchemy.dialects.postgresql import JSONB, ARRAY, VECTOR
...
class Article(Base):
    ...
    # Relationships
    tickers: Mapped[List["ArticleTicker"]] = relationship(back_populates="article_rel", cascade="all, delete-orphan")
    embedding: Mapped[Optional["ArticleEmbed"]] = relationship(back_populates="article_rel", cascade="all, delete-orphan")
    signal_contributions: Mapped[List["SignalContrib"]] = relationship(back_populates="article_rel")
...
class ArticleEmbed(Base):
    ...
    article_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("article.id", ondelete="CASCADE"), primary_key=True)
    ...
    __table_args__ = (
        Index('article_embed_embedding_idx', 'embedding', postgresql_using='ivfflat', 
...
class ArticleTicker(Base):
    ...
    article_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("article.id", ondelete="CASCADE"), primary_key=True)
    ticker: Mapped[str] = mapped_column(String(10), ForeignKey("ticker.symbol"), primary_key=True)
    ...
    __table_args__ = (
        Index('idx_article_ticker_ticker', 'ticker'),
    )
...
class Signal(Base):
    ...
    contributions: Mapped[List["SignalContrib"]] = relationship(back_populates="signal_rel", cascade="all, delete-orphan")
...
class SignalContrib(Base):
    ...
    signal_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("signal.id", ondelete="CASCADE"), primary_key=True)
    article_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("article.id", ondelete="CASCADE"), primary_key=True)
