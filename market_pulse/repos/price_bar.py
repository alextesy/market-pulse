...
class PriceBarRepository(BaseRepository[PriceBar]):
    ...
    def get_bars_by_ticker(self, ticker: str, start_ts: Optional[datetime] = None,
                          end_ts: Optional[datetime] = None, timeframe: Optional[str] = None,
                          limit: int = 1000) -> List[PriceBar]:
    ...
    def get_latest_bar(self, ticker: str, timeframe: str = "1d") -> Optional[PriceBar]:
        ...
        with get_db_session_readonly() as session:
            return session.query(PriceBar).filter(
                and_(
                    PriceBar.ticker == ticker,
                    PriceBar.timeframe == timeframe
                )
            ).order_by(desc(PriceBar.ts)).first()
    
    def get_bars_by_timeframe(self, ticker: str, timeframe: str, limit: int = 1000) -> List[PriceBar]:
        ...
        with get_db_session_readonly() as session:
            ...
            ).order_by(desc(PriceBar.ts)).limit(limit).all()
    
    def get_bars_by_date_range(self, start_date: datetime, end_date: datetime,
                              tickers: Optional[List[str]] = None, timeframe: Optional[str] = None,
                              limit: int = 10000) -> List[PriceBar]:
    ...
    def get_ohlcv_data(self, ticker: str, start_ts: datetime, end_ts: datetime,
                       timeframe: str = "1d") -> List[Dict[str, Any]]:
        """Get OHLCV data for a ticker in a specific time range."""
        with get_db_session_readonly() as session:
            bars = session.query(PriceBar).filter(
                and_(
                    PriceBar.ticker == ticker,
                    PriceBar.timeframe == timeframe,
    ...
    def get_price_stats(self, ticker: str, timeframe: str = "1d", 
    ...
    def delete_bars_by_time_range(self, start_ts: datetime, end_ts: datetime) -> int:
        ...
        with self._transaction_with_retry() as session:
            result = session.query(PriceBar).filter(
                and_(
                    PriceBar.ts >= start_ts,
                    PriceBar.ts <= end_ts
                )
    ...
    def get_data_coverage(self) -> Dict[str, Any]:
        ...
        with get_db_session_readonly() as session:
            ...
            timeframe_counts = session.query(
                PriceBar.timeframe,
                session.query(PriceBar).filter(PriceBar.timeframe == PriceBar.timeframe).count()
            ).distinct().all()
