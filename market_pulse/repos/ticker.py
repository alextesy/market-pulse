...
from ..db.models import Ticker, ArticleTicker
    ...
    def get_active_tickers(self, as_of_date: Optional[datetime] = None) -> List[Ticker]:
        ...
        with get_db_session_readonly() as session:
            ...
            if as_of_date:
                query = query.filter(
                    and_(
                        (Ticker.valid_from.is_(None) | (Ticker.valid_from <= as_of_date)),
    ...
    def find_by_alias(self, alias: str) -> Optional[Ticker]:
        ...
        with get_db_session_readonly() as session:
            ...
            tickers = session.query(Ticker).filter(
                Ticker.aliases.contains({"aliases": [alias]})
            ).all()
    ...
    def get_tickers_with_articles(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        with get_db_session_readonly() as session:
            result = session.query(
                Ticker,
                session.query(ArticleTicker).filter(ArticleTicker.ticker == Ticker.symbol).count().label('article_count')
            ).filter(
                session.query(ArticleTicker).filter(ArticleTicker.ticker == Ticker.symbol).exists()
            ).order_by(
                text('article_count DESC')
            ).limit(limit).all()
            
            return [{"ticker": ticker, "article_count": count} for ticker, count in result]
    ...
    def get_ticker_stats(self) -> Dict[str, Any]:
        ...
        with get_db_session_readonly() as session:
            ...
            exchange_counts = session.query(
                Ticker.exchange,
                session.query(Ticker).filter(Ticker.exchange == Ticker.exchange).count()
            ).filter(Ticker.exchange.isnot(None)).distinct().all()
            
            return {
                "total_tickers": total_count,
                "by_exchange": dict(exchange_counts)
            }
