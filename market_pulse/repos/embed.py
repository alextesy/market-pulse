...
from ..db.models import ArticleEmbed, Article
...
class EmbedRepository(BaseRepository[ArticleEmbed]):
    ...
    def upsert(self, article_id: int, dto: EmbeddingDTO) -> ArticleEmbed:
        ...
        with self._transaction_with_retry() as session:
            ...
            existing = session.query(ArticleEmbed).filter(ArticleEmbed.article_id == article_id).first()
    ...
    def get_by_article_id(self, article_id: int) -> Optional[ArticleEmbed]:
        ...
        with get_db_session_readonly() as session:
            return session.query(ArticleEmbed).filter(ArticleEmbed.article_id == article_id).first()
    
    def find_similar_articles(self, embedding: List[float], limit: int = 10, threshold: float = 0.7) -> List[Tuple[ArticleEmbed, float]]:
        """Find articles with similar embeddings using cosine similarity."""
        with get_db_session_readonly() as session:
            # Use vector cosine similarity
            query = session.query(
                ArticleEmbed,
                func.cosine_similarity(ArticleEmbed.embedding, embedding).label('similarity')
            ).filter(
                func.cosine_similarity(ArticleEmbed.embedding, embedding) > threshold
            ).order_by(
    ...
    def find_similar_articles_by_article_id(self, article_id: int, limit: int = 10, threshold: float = 0.7) -> List[Tuple[ArticleEmbed, float]]:
        """Find articles similar to a given article."""
        with get_db_session_readonly() as session:
            # Get the source article's embedding
            source_embed = session.query(ArticleEmbed).filter(ArticleEmbed.article_id == article_id).first()
            if not source_embed:
                return []
            ...
            query = session.query(
                ArticleEmbed,
                func.cosine_similarity(ArticleEmbed.embedding, source_embed.embedding).label('similarity')
            ).filter(
                and_(
                    ArticleEmbed.article_id != article_id,
                    func.cosine_similarity(ArticleEmbed.embedding, source_embed.embedding) > threshold
                )
            ).order_by(
                text('similarity DESC')
            ).limit(limit)
    ...
    def get_recent_embeddings_for_ticker(self, ticker: str, since_ts: datetime, limit: int = 100) -> List[ArticleEmbed]:
        """Get recent embeddings for articles mentioning a specific ticker."""
        with get_db_session_readonly() as session:
            return session.query(ArticleEmbed).join(Article).join(
                Article.tickers
            ).filter(
                and_(
    ...
    def get_embeddings_by_model(self, model: str, limit: int = 100) -> List[ArticleEmbed]:
        """Get embeddings by model type."""
        with get_db_session_readonly() as session:
            return session.query(ArticleEmbed).filter(
                ArticleEmbed.model == model
            ).limit(limit).all()
    ...
    def get_embedding_stats(self) -> dict:
        ...
        with get_db_session_readonly() as session:
            ...
            model_counts = session.query(
                ArticleEmbed.model,
                session.query(ArticleEmbed).filter(ArticleEmbed.model == ArticleEmbed.model).count()
            ).distinct().all()
    ...
    def delete_embeddings_by_article_ids(self, article_ids: List[int]) -> int:
        ...
        with self._transaction_with_retry() as session:
            result = session.query(ArticleEmbed).filter(
                ArticleEmbed.article_id.in_(article_ids)
