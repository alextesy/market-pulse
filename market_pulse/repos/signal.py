...
from ..models.dto import SignalDTO, SignalContribDTO
...
class SignalRepository(BaseRepository[Signal]):
    ...
    def get_signals_by_ticker(self, ticker: str, start_ts: Optional[datetime] = None, 
                             end_ts: Optional[datetime] = None, limit: int = 100) -> List[Signal]:
    ...
    def get_latest_signal(self, ticker: str) -> Optional[Signal]:
        ...
        with get_db_session_readonly() as session:
            ...
            ).order_by(desc(Signal.ts)).first()
    
    def get_signals_by_score_threshold(self, threshold: float, limit: int = 100) -> List[Signal]:
        """Get signals above a certain score threshold."""
        with get_db_session_readonly() as session:
            return session.query(Signal).filter(
                Signal.score >= threshold
            ).order_by(desc(Signal.score)).limit(limit).all()
    
    def get_signals_by_event_tags(self, event_tags: List[str], limit: int = 100) -> List[Signal]:
    ...
    def get_signal_with_contributions(self, signal_id: int) -> Optional[Dict[str, Any]]:
        ...
        with get_db_session_readonly() as session:
            ...
            contributions = session.query(SignalContrib).filter(
                SignalContrib.signal_id == signal_id
            ).order_by(SignalContrib.rank).all()
    ...
    def get_signal_contributions(self, signal_id: int) -> List[SignalContrib]:
        ...
        with get_db_session_readonly() as session:
            return session.query(SignalContrib).filter(
                SignalContrib.signal_id == signal_id
            ).order_by(SignalContrib.rank).all()
    
    def get_signals_by_time_range(self, start_ts: datetime, end_ts: datetime, 
                                 tickers: Optional[List[str]] = None, limit: int = 1000) -> List[Signal]:
    ...
    def get_signal_stats(self, ticker: Optional[str] = None) -> Dict[str, Any]:
        ...
        with get_db_session_readonly() as session:
            ...
            # Get average scores
            avg_sentiment = session.query(session.query(Signal.sentiment).filter(
                Signal.sentiment.isnot(None)
            ).subquery().columns[0].avg()).scalar()
            
            avg_novelty = session.query(session.query(Signal.novelty).filter(
                Signal.novelty.isnot(None)
            ).subquery().columns[0].avg()).scalar()
            
            avg_velocity = session.query(session.query(Signal.velocity).filter(
                Signal.velocity.isnot(None)
            ).subquery().columns[0].avg()).scalar()
