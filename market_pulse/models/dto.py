...
class IngestItem(BaseModel):
    ...
    lang: Annotated[
        str, StringConstraints(to_lower=True, min_length=2, max_length=5)
    ]  # ISO-639-1/BCP-47
...
class EmbeddingDTO(BaseModel):
    ...
    def validate_dims_match_embedding(self) -> "EmbeddingDTO":
        ...
        if self.dims != len(self.embedding):
            raise ValueError(
                f"dims ({self.dims}) must match embedding length ({len(self.embedding)})"
            )
        return self
