...
def main():
    ...
    if not test_connection():
        print(
            "Database connection not available. Please set POSTGRES_URL environment variable."
        )
        return
    ...
    recent_embeddings = embed_repo.get_recent_embeddings_for_ticker(
        "AAPL", since_ts, limit=10
    )
    ...
    ohlcv_data = price_bar_repo.get_ohlcv_data(
        "AAPL", now - timedelta(days=2), now + timedelta(days=1), "1d"
    )
