...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_article_published_at"), "article", ["published_at"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column("embedding", sa.Text(), nullable=True),  # Will be converted to VECTOR
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
            server_default=sa.text("now()"),
    ...
    op.create_index(
        op.f("ix_article_embed_article_id"),
        "article_embed",
        ["article_id"],
        unique=False,
    )
    ...
    op.create_index(
        op.f("ix_article_ticker_article_id"),
        "article_ticker",
        ["article_id"],
        unique=False,
    )
    op.create_index(
        op.f("ix_article_ticker_ticker"), "article_ticker", ["ticker"], unique=False
    )
    ...
    op.create_index(
        op.f("ix_price_bar_ticker_ts"), "price_bar", ["ticker", "ts"], unique=False
    )
    ...
    op.create_index(
        op.f("ix_signal_ticker_ts"), "signal", ["ticker", "ts"], unique=False
    )
    ...
    op.execute(
        "ALTER TABLE article_embed ALTER COLUMN embedding TYPE vector(384) "
        "USING embedding::vector(384)"
    )
    op.execute(
        "CREATE INDEX IF NOT EXISTS idx_article_embed_embedding ON article_embed "
