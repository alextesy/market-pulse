...
def req(method, url, token, data=None):
    headers = {"Accept": "application/vnd.github+json", "User-Agent": "issue-bulk-uploader"}
    if token:
    ...
    try:
        ...
        if e.code == 403:
            print("\nPERMISSION ERROR: Your token may not have the required scopes.", file=sys.stderr)
            print("Required scopes: 'repo' (for private repos) or 'public_repo' (for public repos)", file=sys.stderr)
...
def get_or_create_milestone(repo, token, title):
    ...
    for state in ("open", "closed"):
        try:
            ms = req("GET", f"{API}/repos/{repo}/milestones?state={state}&per_page=100", token)
            for m in ms:
                if m["title"] == title:
                    return m["number"]
        except urllib.error.HTTPError as e:
            if e.code == 403:
                print(f"WARNING: Cannot access milestones due to permission error. Skipping milestone creation.", file=sys.stderr)
                return None
            raise
    # Create
    try:
        m = req("POST", f"{API}/repos/{repo}/milestones", token, {"title": title, "state": "open"})
        return m["number"]
    except urllib.error.HTTPError as e:
        if e.code == 403:
            print(f"WARNING: Cannot create milestone '{title}' due to permission error. Skipping.", file=sys.stderr)
            return None
        raise
...
def get_or_create_label(repo, token, name):
    ...
    try:
        ...
        if e.code == 404:
            ...
            print(f"WARNING: Cannot access labels due to permission error. Skipping label creation.", file=sys.stderr)
            return name
        else:
            raise
    ...
    color = "ededed"
    try:
        l = req("POST", f"{API}/repos/{repo}/labels", token, {"name": name, "color": color})
        return l["name"]
    except urllib.error.HTTPError as e:
        if e.code == 403:
            print(f"WARNING: Cannot create label '{name}' due to permission error. Skipping.", file=sys.stderr)
            return name
        else:
...
def issue_exists(repo, token, title):
    ...
    try:
        ...
        if e.code == 403:
            print(f"WARNING: Cannot check existing issues due to permission error. Assuming issue doesn't exist.", file=sys.stderr)
            return False
        raise

def create_issue(repo, token, item):
    milestone_no = get_or_create_milestone(repo, token, item["milestone"]) if item.get("milestone") else None
    labels = []
    ...
    try:
        ...
        if e.code == 403:
            print(f"ERROR: Cannot create issue '{item['title']}' due to permission error.", file=sys.stderr)
            print("Make sure your token has 'repo' scope for private repos or 'public_repo' for public repos.", file=sys.stderr)
        else:
            print(f"ERROR: Failed to create issue '{item['title']}': {e}", file=sys.stderr)
...
def main():
    ...
    if not args.token:
        ...
        print("3. Select scopes: 'repo' (private repos) or 'public_repo' (public repos)", file=sys.stderr)
        print("4. Copy the token and set it as GITHUB_TOKEN environment variable", file=sys.stderr)
        sys.exit(1)
